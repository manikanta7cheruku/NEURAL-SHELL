  ...
  return (
    <div className="max-w-2xl space-y-10">
      <div className="space-y-2">
        ...
        <h2 className="text-[28px] font-bold text-white tracking-tight">Vocal Interface</h2>
      </div>

      <div className="space-y-2">
        <label className="text-[10px] text-white/50 tracking-[0.2em] font-medium uppercase">Wake Word</label>
        <input type="text" value="seven" disabled className="w-full px-4 py-3.5 bg-white/[0.02] border border-white/[0.05] rounded-lg text-white/30 text-[13px] font-mono cursor-not-allowed outline-none" />
        ...
        <div className="grid grid-cols-2 gap-3 max-h-56 overflow-y-auto pr-2 scrollbar-thin">
          {voices.map(v => (
            <div key={v.index} onClick={() => setField('voiceIndex', v.index)}
                 className={`p-4 rounded-xl border cursor-pointer transition-all duration-200 ${data.voiceIndex === v.index ? 'bg-white/[0.08] border-white/40' : 'bg-white/[0.02] border-white/[0.08] hover:bg-white/[0.05]'}`}>
              ...
              <div className="text-[10px] text-white/40 font-mono flex justify-between items-center">
                ...
                <button onClick={(e) => { e.stopPropagation(); previewVoice(v.voice_id); }} disabled={voicePreviewPlaying}
      ...
      <div className="flex gap-3 pt-4 border-t border-white/[0.05]">
        ...
        <button onClick={next} className="flex-1 py-3.5 bg-white text-black text-[12px] font-semibold rounded-lg hover:bg-white/90 transition-colors">Continue</button>
      </div>
    </div>
  );
}