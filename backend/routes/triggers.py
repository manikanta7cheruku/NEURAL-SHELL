...
class LayoutRequest(BaseModel):
    layout:          str         # maximize | split_vertical | split_horizontal | grid | stack
    hwnd_list:       list = []   # preferred: exact window handles from arrangement card
    minimize_hwnds:  list = []   # windows user unchecked — minimize these
    app_names:       list = []   # fallback: app name strings (legacy)


@router.post("/api/triggers/layout")
def apply_layout(body: LayoutRequest):
    """
    Apply window arrangement to selected windows.
    Prefers hwnd_list (exact handles from arrangement card).
    Falls back to app_names for legacy callers.
